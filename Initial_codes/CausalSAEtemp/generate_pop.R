...
#' @param coef_outcome_p_score List of parameters to generate covariates present in 
#' outcome and propensity score regression, that is: 
#' \itemize{
#' \item coef_norm_outcome
#' \item coef_norm_p_score
#' \item coef_lognorm_outcome
#' \item coef_lognorm_p_score
#' \item coef_unif_outcome
#' \item coef_unif_p_score
#' }
#' @param coef_outcome List of parameters to generate covariates in 
#' outcome regression, that is: 
#' \itemize{
#' \item coef_norm
#' \item coef_lognorm
#' \item coef_unif
#' }
#' @param frac_out Fraction of outlying observations
#' @param type_regression Type of regression
#' @param cov_norm Variance - covariance matrix to generate correlated covariates,
#'  default: cov_norm = NULL
#' @param start_seed Seed for simulations, default: start_seed = 1
#'
#'
#' @return 
#' 
...
generate_pop <- function(coef_outcome_p_score = get_default_cov("outcome_p_score"),
  ...
  ## Check cov_outcome
  Xot <- generate_X(n = N, reg_coef = coef_outcome_p_score, 
                    cov_norm = NULL, 
                    type_regression = "outcome_p_score", 
                    start_seed = 1)
  

  Xo <- generate_X(n = N, reg_coef = coef_outcome, 
                   cov_norm = NULL, 
                   type_regression = "outcome", 
                   start_seed = 1)
  #if not null
  Xt <- generate_X(n = N, reg_coef = coef_p_score, 
                   cov_norm = NULL, 
                   type_regression = "p_score", 
                   start_seed = 1)
...
generate_X <- function(N, reg_coef,
  ...
  if (type == "outcome_p_score") {
    ...
    if (length(reg_coef$coef_norm_outcome) != 0) {
      ...
      X_n <- get_X_norm(n = N, coef_norm = reg_coef$coef_norm_outcome, cov_norm)
    ...
    if (length(reg_coef$coef_lognorm_outcome)!= 0) {

      X_ln <- get_X_lognorm(n = N, coef_lognorm = reg_coef$coef_lognorm_outcome)
      ...
      ####  
      if (is.null(Xreg_outcome)) {
        Xreg_outcome <- X_ln %*% reg_coef$coef_lognorm_outcome 
      } else {
        Xreg_outcome <- Xreg_outcome + X_ln %*% reg_coef$coef_lognorm_outcome 
      }
      
      if (is.null(Xreg_p_score)) {
        Xreg_p_score <- X_ln %*% reg_coef$coef_lognorm_p_score 
      } else {
        Xreg_p_score <- Xreg_p_score + X_ln %*% reg_coef$coef_lognorm_p_score
      }
    } 
    
    if (length(reg_coef$coef_unif_outcome)!= 0) {
      ###
      X_u <- get_X_unif(n = N, coef_unif = reg_coef$coef_unif_outcome)
      ...
      if (is.null(Xreg_outcome)) {
        Xreg_outcome <- X_u %*% reg_coef$coef_unif_outcome 
      } else {
        Xreg_outcome <- Xreg_outcome + X_u %*% reg_coef$coef_unif_outcome 
      }
      
      if (is.null(Xreg_p_score)) {
        Xreg_p_score <- X_u %*% reg_coef$coef_unif_outcome 
      } else {
        Xreg_p_score <- Xreg_p_score + X_u %*% reg_coef$coef_unif_outcome 
      }
    } 
    ...
    if (length(reg_coef$coef_norm) != 0) {
      X_n <- get_X_norm(n = N, coef_norm,  cov_norm)
    ...
    if (length(reg_coef$coef_lognorm)!= 0) {
      ####
      X_ln <- get_X_lognorm(n = N, coef_lognorm = reg_coef$coef_lognorm)
    ...
    if (length(reg_coef$coef_unif)!= 0) {
      ###
      X_u = get_X_unif(n = N, coef_unif = reg_coef$coef_lognorm)
  ...
  if (type == "outcome_p_score") {
    ...
    output <- list(Xcov = Xcov, 
                   ...
                   Xreg_p_score = Xreg_p_score)
  } else if (type == "outcome") { 
    names(Xcov) <-  paste0("Xo", 1:ncol(Xcov)) 
    output <- list(Xcov = Xcov, Xreg = Xreg)
  } else { 
...
#' Check coefficients of covariates
#'
#'
#' @param coef_outcome
#' @param coef_p_score
...
check_coef <- function(coef_outcome, coef_p_score, coef_outcome_p_score) {
  ...
  for (name in c(names(coef_outcome), names(coef_p_score), names(coef_outcome_p_score))) {
    if (name %!in% c("intercept","coef_norm",
                     ...
                     "coef_norm_outcome", "coef_unif_outcome", 
                     "coef_lognorm_outcome", 
                     "coef_norm_p_score", "coef_unif_p_score", 
                     "coef_lognorm_p_score") ) {
