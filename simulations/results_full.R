setwd("C:/Users/katar/Documents/GitHub/causalSAE/simulations/results/Mplus_2")
library(data.table)
...
plot(1:ncol(RBP), ARBP[54,], type = "l")
lines(1:ncol(RBP), ARBP[33,], type = "l", col = 2)
...
#d <- dt_results[, lapply(.SD, mean), by = "Method"]
#divide by abs bias
#dt_cluster1 <- data.table(cluster1 = dt_results$cluster1,
#                          Method = dt_results$Method)

output_summary <- dt_results[, .(mean_c1 = mean(cluster1),
                                 mean_c2 = mean(cluster2)),
                             by = c("Method")]
