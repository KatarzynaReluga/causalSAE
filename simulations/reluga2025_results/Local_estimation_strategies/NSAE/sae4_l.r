...
stop_time = Sys.time() #Time difference of 1.404107 hours
...
for (k in 1:dim(AIPW_names)[1]) {
  ...
  assign(paste0(paste0(unlist(unname(AIPW_names[k, ])), collapse = ""), "L"), 
         AIPWf$tau)
...
for (k in 1:dim(IPW_names)[1]) {
  ...
  assign(paste0(paste0(unlist(unname(IPW_names[k, ])), collapse = ""), "L"), IPWf$tau)
}
...
outputName = paste("sae4_L_", SN, ".RData", sep = "")
