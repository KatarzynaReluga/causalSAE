...
for (k in 1:dim(AIPW_names)[1]) {
  ...
  assign(paste0(paste0(unlist(unname(AIPW_names[k, ])), collapse = ""), "Rc"), 
         AIPWf$tau)
...
for (k in 1:dim(IPW_names)[1]) {
  ...
  assign(paste0(paste0(unlist(unname(IPW_names[k, ])), collapse = ""), "Rc"), IPWf$tau)
}
...
Results = list(ni = ni,
               ...
               Xt_Rc = Xt)
stop_time = Sys.time() 
outputName = paste("sae4_Rc_", SN, ".RData", sep = "")
