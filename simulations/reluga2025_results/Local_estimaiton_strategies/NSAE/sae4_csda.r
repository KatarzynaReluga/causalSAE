...
stop_time = Sys.time() #Time difference of 11 mins
...
for (k in 1:dim(AIPW_names)[1]) {
  ...
  assign(paste0(paste0(unlist(unname(AIPW_names[k, ])), collapse = ""), "csda"), 
         AIPWf$tau)
...
for (k in 1:dim(IPW_names)[1]) {
  ...
  assign(paste0(paste0(unlist(unname(IPW_names[k, ])), collapse = ""), "csda"), IPWf$tau)
}
...
C = Sys.time()
stop_time = Sys.time() 
outputName = paste("sae4_csda_", SN, ".RData", sep = "")
