...
for (k in 1:dim(AIPW_names)[1]) {
  ...
  assign(paste0(paste0(unlist(unname(AIPW_names[k, ])), collapse = ""), "M"), 
         AIPWf$tau)
...
for (k in 1:dim(IPW_names)[1]) {
  ...
  assign(paste0(paste0(unlist(unname(IPW_names[k, ])), collapse = ""), "M"), IPWf$tau)
}
...
outputName = paste("sae4_M_", SN, ".RData", sep = "")
