...
for (k in 1:dim(AIPW_names)[1]) {
  ...
  assign(paste0(paste0(unlist(unname(AIPW_names[k, ])), collapse = ""), "R"), 
         AIPWf$tau)
...
for (k in 1:dim(IPW_names)[1]) {
  ...
  assign(paste0(paste0(unlist(unname(IPW_names[k, ])), collapse = ""), "R"), IPWf$tau)
}
...
C = Sys.time()
stop_time = Sys.time() 
outputName = paste("sae4_R_", SN, ".RData", sep = "")
