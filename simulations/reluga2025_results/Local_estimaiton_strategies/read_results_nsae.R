...
# 41
tau_trueM <- dt_Mean[c(4:51), c(125:165)]
#tau_trueM <- dt_Mean[c(4:66), c(125:165)]

Methods <- dt_res$Method[4:51]
#Methods <- dt_res$Method[4:66]




# Bias -----------------------------------------------
B <- dt_Mean[c(4:51), c(43:83)]
BMean <- rowMeans(B)
...
# E
results_2E <- data.frame(Method = Methods,
                          mse = MSEMean,
                          Bias = BMean,
                          ABias = ABMean,
                          RABias = RBMean, 
...
resultsF <- rbind(results_2E, results_csda)

resultsF$Impute <- c(rep("E", 63),
                     rep("L", 63),
                     rep("M", 63),
                     rep("Mq", 63),
...
resultsF$Impute <- c(rep("2E", 48),
                    rep("csda", 48))

plot(as.numeric(tau_trueM[1,]), type = "l", ylim = c(0.1, 2), lwd = 2)
lines(ResultsM$M_csda, col = 2)
lines(ResultsM$Lcsda, col = 3)
lines(ResultsM$MXtcsda, col = 4)

results_Ford <- resultsF[order(resultsF$rrmse, decreasing = FALSE), ]  

write.csv(resultsF,
          file = 'results_local_2e_csda.csv', row.names = FALSE)

#########
# Plots
res_nsae <- read.csv('results_nsae4_all.csv')
res_sae <- read.csv('results_sae4.csv')
