...
# 41
tau_trueM <- dt_Mean[c(4), c(125:165)]

Methods <- dt_res$Method[4]

#RABiasR <- mean(abs(colMeans(R - tau_true)/tau_true[1, ]))
#BiasR <- mean(colMeans(R - tau_true))
#ABiasR <- mean(abs(colMeans(R - tau_true)))
#mseR <- mean(colMeans((R - tau_true)^2))

####################################################
# D = 25
# Bias -----------------------------------------------
B <- dt_Mean[c(4), c(43:83)]
BMean <- rowMeans(B)
...
# MSE --------------------------------
MSE <- dt_Mean[c(4), c(84:124)]
MSEMean <- rowMeans(MSE)
MSEMedian <- apply(MSE, 1, quantile, probs = 0.5)

RRMSE <- sqrt(MSE)/abs(tau_trueM)
RRMSEMean <- rowMeans(RRMSE)
...
plot(as.numeric(tau_trueM[1,]), type = "l", ylim = c(0.1, 2))
lines(ResultsM$LPX, col = 2)
lines(ResultsM$MPX, col = 3)
lines(ResultsM$PX, col = 4)

results_Ford <- resultsF[order(resultsF$mse, decreasing = FALSE), ]  

write.csv(resultsF,
          file = 'results_local_model_TMLE.csv', row.names = FALSE)
