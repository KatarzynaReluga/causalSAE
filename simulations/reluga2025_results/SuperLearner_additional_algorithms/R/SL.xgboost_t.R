...
#'
#' @export
SL.xgboost_t = function(Y, X, newX, family, obsWeights, id, ntrees = 100,
  ...
  if (family$family == "gaussian") {
    ...
    model = xgboost::xgboost(data = xgmat, objective=objective, nrounds = best_iter_xgb,
                max_depth = max_depth, min_child_weight = minobspernode, eta = shrinkage,
                verbose = verbose, nthread = nthread, params = params,
                save_period = save_period)
  }
  if (family$family == "binomial") {
    model = xgboost::xgboost(data = xgmat, objective="binary:logistic", nrounds = best_iter_xgb,
                max_depth = max_depth, min_child_weight = minobspernode, eta = shrinkage,
                verbose = verbose, nthread = nthread, params = params,
  ...
  if (family$family == "multinomial") {
    # TODO: test this.
    model = xgboost::xgboost(data = xgmat, objective="multi:softmax", nrounds = best_iter_xgb,
                max_depth = max_depth, min_child_weight = minobspernode, eta = shrinkage,
...
#' @param ... Any remaining arguments (not supported though).
predict.SL.xgboost_t <- function(object, newdata, family, ...) {
  .SL.require("xgboost")
  if(packageVersion("xgboost") < 0.6) stop("SL.xgboost requires xgboost version >= 0.6, try help(\'SL.xgboost\') for details")
