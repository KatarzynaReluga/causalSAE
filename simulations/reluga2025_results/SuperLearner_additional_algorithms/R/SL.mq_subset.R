...
#' 
SL.mq_subset <- function(Y, X, newX, family, obsWeights, model = FALSE, ...) {
  
  # X must be a dataframe, not a matrix.
  if (is.matrix(X)) {
    X = as.data.frame(X)
  }
  namesX <- paste0("X", 1:10)
  #vars <- names(X[ ,!names(X) %in% c("group")])
  vars <- names(X[ ,names(X) %in% namesX])
...
#' 
#' 
predict.SL.mq_subset <- function(object, newdata, ...) {
