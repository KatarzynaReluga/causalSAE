...
#######################################################################
# Question: Do we need to standardize age? Check the results with age. 
#######################################################################
...
modglme <-
  glmer(
    treatment ~ age + factor(gender) + factor(education) + factor(statciv) +
...
for (i in ar) {
  maxi <-
    min(max(p_hat[dd$treatment == 0 &
                    dd$region == i]), max(p_hat[dd$treatment == 1 & dd$region == i]))
  mini <-
    max(min(p_hat[dd$treatment == 1 &
                    dd$region == i]), min(p_hat[dd$treatment == 0 & dd$region == i]))
  ind <-
    c(ind, (p_hat[dd$region == i] <= maxi) * (p_hat[dd$region == i] >= mini))
}
...
plot(
  density(p_hat[dd$treatment == 0]),
  col = "red",
  type = "l",
  lwd = 2,
  main = "Common support in the full sample after trimming",
...
lines(density(p_hat[dd$treatment == 1]),
      col = "blue",
      type = "l",
      lwd = 2)
legend(
...
l.c.bar <-
  tapply(l.p.hat[dd$treatment == 0], dd$region[dd$treatment == 0], mean)
l.t.bar <-
  tapply(l.p.hat[dd$treatment == 1], dd$region[dd$treatment == 1], mean)
s.l.c <-
  tapply(l.p.hat[dd$treatment == 0], dd$region[dd$treatment == 0], sd)
s.l.t <-
  tapply(l.p.hat[dd$treatment == 1], dd$region[dd$treatment == 1], sd)
...
for (i in ar) {
  ...
  s.out <-
    matchit(
      treatment ~ age + factor(gender) + factor(education) + factor(statciv) +
        factor(tenure) + factor(familytype),
      "subclass",
      subclass = 10 ,
      data = sub_dd
    )
...
for (i in ar) {
  True[i] <-
    (dd$treatment[dd$region == i] %*% (dd$y[dd$region == i] / p_hat[dd$region ==
                                                                      i])) / (sum(dd$treatment[dd$region == i] / p_hat[dd$region == i])) -
    ((1 - dd$treatment[dd$region == i]) %*% (dd$y[dd$region == i] / (1 -
                                                                       p_hat[dd$region == i]))) / (sum((1 - dd$treatment[dd$region == i]) / (1 -
                                                                                                                                               p_hat[dd$region == i])))
}
True <- na.omit(True)

True_total <-
  (dd$treatment %*% (dd$y / p_hat)) / (sum(dd$treatment / p_hat)) -
  ((1 - dd$treatment) %*% (dd$y / (1 - p_hat))) / (sum((1 - dd$treatment) / (1 -
                                                                               p_hat)))

## Original estimates of the effects
tau.original <-
  rbind(cbind(Ni, Nc, Nt, na.omit(True)), c(N, sum(Nc), sum(Nt), True_total))
NUT2 <-
  c(
    "Piemonte",
    "Valle-d'Aosta",
...
est.orig <-
  data.frame(
    "NUTS2" = NUT2,
    "Original sample size" = tau.original[, 1],
    "Treated percentage" = round(tau.original[, 3] /
                                   tau.original[, 2], 2),
    "Original estimate" = round(tau.original[, 4], 2)
  )
...
{
  tau_hat_01 <-
    vector(mode = "numeric", length = length(ar)) # ATE using only the sample (classical approach)
  tau_hat_02 <-
    vector(mode = "numeric", length = length(ar)) # ATE SAE-EBLUP method
  tau_hat_03 <-
    vector(mode = "numeric", length = length(ar)) # ATE SAE-MQ method
  ...
  while (test == "try-error" | test1 == "try-error" | test2 < 32)
  {
    ...
    X <-
      cbind(
        1,
        samp$age,
        model.matrix( ~ factor(samp$gender))[, -1],
        model.matrix( ~ factor(samp$education))[, -1],
        model.matrix( ~ factor(samp$statciv))[, -1],
        model.matrix( ~ factor(samp$tenure))[, -1],
        model.matrix( ~ factor(samp$familytype))[, -1]
      )
    pX <-
      cbind(
        1,
        dd$age,
        model.matrix( ~ factor(dd$gender))[, -1],
        model.matrix( ~ factor(dd$education))[, -1],
        model.matrix( ~ factor(dd$statciv))[, -1],
        model.matrix( ~ factor(dd$tenure))[, -1],
        model.matrix( ~ factor(dd$familytype))[, -1]
      )
    m_p <-
      try(glm.mq.binom(
        y = dd$treatment,
    ...
    for (i in ar) {
      MQ_p[which(ar == i)] = try((mean(scores[, 2][scores[, 1] == i])), silent =
                                   TRUE)
    }
    m_p1 <-
      try(glm.mq.binom(
    ...
    Xy <-
      cbind(
        ...
        model.matrix( ~ factor(samp$familytype))[, -1] ,
        samp$noroom ,
        model.matrix( ~ factor(samp$dtype))[, -1],
        model.matrix( ~ factor(samp$crime))[, -1],
        samp$dincome,
        samp$treatment
      )
    test2 <- ncol(Xy)
  ...
  pXy <-
    cbind(
      ...
      model.matrix( ~ factor(dd$familytype))[, -1] ,
      dd$noroom ,
      model.matrix( ~ factor(dd$dtype))[, -1],
      model.matrix( ~ factor(dd$crime))[, -1],
      dd$dincome,
      dd$treatment
    )
  ...
  m_y <-
    QRLM(
      ...
      k = 1.345,
      maxit = 100
    ) # if we assume the treatment effect is different for each area and that the differnces are at random then we have to include the random slope for treatment status
  qo <-
    matrix(c(gridfitinter(
      samp$y, m_y$fitted.values, m_y$q.values
    )), nrow = n, ncol = 1)
  ...
  for (i in 1:length(ar)) {
    tau_hat_03[i] <-
      (dd$treatment[dd$region == ar[i]] %*% (y_hat[dd$region == ar[i]] / p_hat[dd$region ==
                                                                                 ar[i]])) / (sum(dd$treatment[dd$region == ar[i]] / p_hat[dd$region == ar[i]])) -
      ((1 - dd$treatment[dd$region == ar[i]]) %*% (y_hat[dd$region == ar[i]] / (1 -
                                                                                  p_hat[dd$region == ar[i]]))) / (sum((1 - dd$treatment[dd$region == ar[i]]) / (1 -
                                                                                                                                                                  p_hat[dd$region == ar[i]])))
  ...
  modlme <-
    lmer(
      y ~ age + factor(gender) + factor(education) + factor(statciv) + factor(tenure) + wintens + factor(familytype) + noroom +
        factor(dtype) + factor(crime) + dincome + treatment + (1 + treatment ||
                                                                 region),
      data = samp
    )
  modglme <-
    glmer(
      treatment ~ age + factor(gender) + factor(education) + factor(statciv) +
        factor(tenure) + factor(familytype) + (1 |
                                                 region),
      data = dd,
  ...
  y_hat[s[, 2]] <- samp$y
  y_hat[-s[, 2]] <-
    predict(modlme, newdata = non.samp, allow.new.levels = TRUE)
  
  for (i in 1:length(ar)) {
    tau_hat_01[i] <-
      (samp$treatment[samp$region == ar[i]] %*% (samp$y[samp$region == ar[i]] /
                                                   p_hat[s[s$region == ar[i], 2]])) / (sum(samp$treatment[samp$region == ar[i]] /
                                                                                             p_hat[s[s$region == ar[i], 2]])) -
      ((1 - samp$treatment[samp$region == ar[i]]) %*% (samp$y[samp$region ==
                                                                ar[i]] / (1 - p_hat[s[s$region == ar[i], 2]]))) / (sum((1 - samp$treatment[samp$region ==
                                                                                                                                             ar[i]]) / (1 - p_hat[s[s$region == ar[i], 2]])))
  ...
  Direct[h, ] <- tau_hat_01
  EBLUP[h, ] <- tau_hat_02
  print(c(h, ":", sum(is.na(tau_hat_01)) , ":", sum((
    table(samp$region, by = samp$treatment) == 0
  ))))
} #simulation ends
...
cbind(
  "NUTS2" = NUT2[-20],
  True,
  "IPW_Direct" = apply(Direct, 2, mean, na.rm = TRUE),
  "IPW_EBLUP" = apply(EBLUP, 2, mean),
  "IPW_MQ" = apply(MQ, 2, mean)
)
RB.EBLUP <- (apply(EBLUP, 2, mean) - True) / abs(True) * 100
RB.MQ <- (apply(MQ, 2, mean) - True) / abs(True) * 100
RB.Direct <- (apply(Direct, 2, mean, na.rm = TRUE) - True) / abs(True) *
  100
summary(RB.Direct)
summary(RB.EBLUP)
summary(RB.MQ)
...
RRMSE.EBLUP <-
  (sqrt(apply(
    apply(EBLUP, 1, function(x) {
      (x - True) ^ 2
    }), 1, mean, na.rm = TRUE
  )) / abs(True)) * 100
RRMSE.MQ <-
  (sqrt(apply(apply(MQ, 1, function(x) {
    (x - True) ^ 2
  }), 1, mean, na.rm = TRUE)) / abs(True)) * 100
summary(RRMSE.Direct)
summary(RRMSE.EBLUP)
summary(RRMSE.MQ)
...
results <-
  data.frame(
    ...
    "True" = na.omit(True),
    "IPW_Direct" = apply(Direct, 2, mean, na.rm = TRUE),
    "(s.e.Direct)" = sqrt(apply(
      apply(Direct, 1, function(x) {
        (x - True) ^ 2
      }), 1, mean, na.rm = TRUE
    )),
    "IPW_EBLUP" = apply(EBLUP, 2, mean),
    "(s.e.EBLUP)" = sqrt(apply(
      apply(EBLUP, 1, function(x) {
        (x - True) ^ 2
      }), 1, mean, na.rm = TRUE
    )),
    "IPW_MQ" = apply(MQ, 2, mean),
    "(s.e.MQ)" = sqrt(apply(apply(MQ, 1, function(x) {
      (x - True) ^ 2
    }), 1, mean, na.rm = TRUE))
  )
...
results <- NULL
CI_Direct <-
  apply(Direct, 2, function(x)
    quantile(x, probs = c(0.025, 0.975), na.rm = TRUE))
CI_EBLUP <-
  apply(EBLUP, 2, function(x)
    quantile(x, probs = c(0.025, 0.975), na.rm = TRUE))
CI_MQ <-
  apply(MQ, 2, function(x)
    quantile(x, probs = c(0.025, 0.975), na.rm = TRUE))
results <-
  data.frame(
    "NUTS2" = NUT2[-20],
...
ggplot(data = results) +
  ...
  geom_hline(yintercept = 0,
             color = "red",
             lty = "dashed") +
  geom_errorbar(mapping = aes(x = NUTS2, ymin = LL_D, ymax = UL_D),
                lty = "dashed") +
  geom_errorbar(mapping = aes(x = NUTS2, ymin = LL_E, ymax = UL_E),
                color = "red") +
  geom_errorbar(mapping = aes(x = NUTS2, ymin = LL_MQ, ymax = UL_MQ),
                color = "blue") +
...
par(mfrow = c(1, 2))
boxplot(
  cbind(Direct = RB.Direct, EBLUP = RB.EBLUP, MQ = RB.MQ),
  main = "Relative Bias",
  cex.axis = 1.5,
  font.lab = 2,
  lwd = 1.5
...
abline(h = 0,
       ...
       lwd = 2)
boxplot(
  cbind(Direct = RRMSE.Direct, EBLUP = RRMSE.EBLUP, MQ = RRMSE.MQ),
  main = "Relative Root Mean Square Error",
  cex.axis = 1.5,
  font.lab = 2,
  lwd = 1.5
)
...
plot(
  ...
  lty = 2,
  col = "black",
  xlab = "area-specific effect",
  main = "Distribution of area-specific ATE - different estimators",
  lwd = 2
)
...
legend(
  1,
  2.5,
  legend = c("True", "IPW_Driect", "IPW_EBLUP", "IPW_MQ"),
...
plot(ar, True, type = "b")
lines(ar,
      apply(Direct, 2, mean, na.rm = TRUE),
      type = "b",
      col = "red")
...
data <-
  data.frame(
    y = c(matrix(Direct, I(1000 * 19), 1), matrix(EBLUP, I(1000 * 19), 1), matrix(MQ, I(1000 *
                                                                                          19), 1)),
    reg = rep(rep(NUT2[-20], each = 1000), 3),
    meth = rep(c("Direct", "EBLUP", "MQ"), each = I(1000 * 19))
  )
data$reg <-
  factor(data$reg)#, levels = c(19,20,21,16,17,18,13,14,15,10,11,12,7,8,9,4,5,6,1,2,3))
#ind <- c(19,16,17,18,13,14,15,10,11,12,7,8,9,4,5,6,1,2,3)
bwplot(
  y ~ meth |
    reg,
  data = data,
  ylab = expression(bold("Causal effects")),
  xlab = expression(bold("IPW Estimators")),
  main = "Causal effects evaluated for each region in Italy",
  layout = c(3, 7),
  panel = function(...) {
